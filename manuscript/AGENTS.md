...
- **Hermes (latest run):** `{{hermes.primary_model}}`, `{{hermes.processed}}`, `{{hermes.success_count}}`, `{{hermes.cache_hits}}`, `{{hermes.run_id}}`, `{{hermes.tokens_total}}`, `{{hermes.tokens_mean}}`, `{{hermes.mean_topic_s}}`, `{{hermes.hermes_lean_compiles_count}}`, `{{hermes.models_used}}`.
