...
def test_write_unified_formalism_appendix_markdown_roundtrip(tmp_path: Path) -> None:
    ...
    out2 = write_full_topic_lean_catalogue_markdown(tmp_path, c)
...
def test_build_topic_latex_equations_markdown_structure() -> None:
    c = FEPTopicCatalogue.from_yaml(PROJ / "config" / "topics.yaml")
    md = build_topic_latex_equations_markdown(c, PROJ)
...
def test_fep_001_latex_equations_from_yaml_matches_data_module() -> None:
    ...
    from topic_latex_equations_data import THEOREM_LATEX
...
def test_hermes_block_counts_model_chain_advances(tmp_path: Path) -> None:
    ...
    which tracks Lean baseline-sketch fallback after a refined-Lean failure.
...
def test_count_test_cases_invalidates_on_test_change(tmp_path: Path) -> None:
    ...
    cache_path.write_text(json.dumps({"collected": 99, "ts": "stub"}), encoding="utf-8")
