...
Set ``FEP_LEAN_CATALOGUE_COMPILE_TEST=1`` to run all 50 ``lake env lean`` checks.
...
@pytest.mark.skipif(
    ...
    reason="Set FEP_LEAN_CATALOGUE_COMPILE_TEST=1 for full 50-topic lake sweep.",
)
def test_all_catalogue_sketches_compile(monkeypatch: pytest.MonkeyPatch) -> None:
