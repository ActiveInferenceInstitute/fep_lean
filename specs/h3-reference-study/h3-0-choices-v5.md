# H3.0 v5 choice ledger

This ledger explains the prospective H3.0 v5 decisions that are not obvious
...
V1, v2, v3, and v4 were WITHHELD. Their versioned protocols, choice ledgers,
tests, review requests, transition records, and disposition histories are
immutable inputs to v5. V4 received exact statistical and model/domain/action
approvals, but its implementation/provenance/outcome-lock review was WITHHOLD.
V5 is the append-only repair candidate. Its review request remains absent until
the candidate, choices, and canonical test are green and byte-frozen; no
downstream gate is open.

The v4 freeze map and WITHHELD record deliberately precede the v5 transition
snapshot in the hash graph. The transition snapshot precedes the v5 package;
the eventual review request will follow the frozen v5 package. No candidate or
request may hash a descendant and create a provenance cycle.

## Why v4 was withheld

The exact reviewer identities and reviewed hashes live in
[`preregistration-v4-withheld.json`](preregistration-v4-withheld.json). The
implementation review identified five substantive repair groups:
...
V5 applies those repairs while retaining the already approved model and
statistical boundary. It also closes two implementation consequences exposed by
replay: oracle values use an independently derived information-form Gaussian
solve, and bootstrap values preserve every returned position and duplicate in
the registered `math.fsum` order.
...
V5 addresses these findings without changing any v4 byte.
...
These refinements take effect only if the exact v5 package is later accepted:
...
consumes v5 and cannot be retried under another identifier; unsafe claim path
