# H3.0 v7 choice ledger

This ledger explains the prospective H3.0 v7 decisions that are not obvious
...
V1 through v6 were WITHHELD. Their versioned packages, requests, freeze maps,
transition records, and dispositions are immutable inputs to v7. The exact v6
terminal history lives in
[`preregistration-v6-withheld.json`](preregistration-v6-withheld.json) and
[`transition-state-snapshot-v7.json`](transition-state-snapshot-v7.json).
No downstream gate is open.

The live candidate is phase-neutral. A future request may be absent before
minting or present during review without changing these candidate bytes. A
request contains no review decision and is never approval; a separate freeze
and exactly one terminal acceptance or WITHHELD descendant records review
disposition. Earlier artifacts never hash those mutable descendants.

## Why v5 was withheld

The three exact v5 reviewers all withheld. Their findings reduce to five
repair principles:

- The candidate and navigation tests must remain valid before and after request
  minting; historical absence declarations are capture-time facts, not
  perpetual live invariants.
- The nonidentifiability control must independently recompute observable
  predictive log density as well as predictive mean and covariance.
- Aggregate confirmatory success must bind the sensitivity decision while
  treating expected negative-control recovery failures as diagnostic evidence,
  not positive-domain failures.
- Future H3.6S collection evidence must contain the exact registered node IDs
  and a terminal pytest summary whose count agrees with that roster.
- Self-cycle prevention needs a fail-closed external sandbox and explicit
  evidence limits; literal path-substring scans are defense in depth, not proof
  that dynamic access did not occur.

V6 made these repairs without changing any v5 byte. V7 retains the v5
implementation/provenance repairs:

- Live H3.6S implementation and package bytes must remain equal to their
  independently reviewed immutable owner snapshot through action freeze,
  development, pre-run, and the post-claim pre-RNG recheck.
- Development data records and passive or active selectors must be replayed
  from the bound random streams and trajectory bytes, rather than trusting
  reported scores or costs.
- All eight protected outputs need exact role-specific schemas, record counts,
  domains, cross-file bindings, and independent recomputation; nonempty
  placeholder JSON cannot complete a run.
- Acceptance, action, pre-run, claim, manifest, result, snapshot, and protected
  path handling needs one no-follow, single-link descriptor authority with
  exact modes, lexical broken-symlink handling, exclusive creation, and honest
  durability limits.
- Strict JSON must reject every nonfinite spelling and exponent overflow, and
  the H3.6S review identity must be nonempty.

Oracle values still use an independently derived information-form Gaussian
solve, and bootstrap values still preserve every returned position and
duplicate in the registered `math.fsum` order.

The earlier review findings still establish three general principles:
...
V7 preserves these findings without changing any v1-v6 byte.

## Why v6 was withheld

V6 reached exact-hash review, but its statistical reviewer found that the
registered BCa effect intervals were computed without becoming decision
authority. Tail fractions could make the Holm family reject while a BCa effect
criterion itself failed. The aggregate decision and claim matrix also listed
only part of their conjunctive upstream evidence.

V7 repairs that gap without changing the model, action, seed, formal-gate, or
one-shot filesystem choices. It introduces one exact passive-20 or active-26
effect-criterion roster before Holm, requires every same-ID effect row and Holm
row to pass together, and resolves every claim activation through an exhaustive
ordered predicate lineage. The scientific seed root and all RNG known answers
remain the exact v6 authority; only their candidate-epoch wrapper advances.
...
The selected H3.3 estimator and the oracle are not two names for one helper.
The oracle uses a separately derived block-information center solve and
canonical information-filter latent recursion, with short literal dense
conditioning checks for both full-rank and rank-two observation operators.
Candidate-only defects must change candidate-to-oracle diagnostics while the
oracle remains executable with candidate helpers disabled.
...
it. Complete owner subpayload digests protect BCa, numeric aggregation, the
independent oracle, SBC multinomial/DKW/binomial, add-one, win, and Holm
mappings against unenumerated leaf drift. Algorithm known answers remain
separate so a correct hash cannot stand in for correct computation.
...
These refinements take effect only if the exact v7 package is later accepted:
...
The future owner gate must run collection and focused execution inside a fresh,
fail-closed external sandbox containing only the registered input tree and
runtime closure. The external harness, not the focused process, writes evidence
after exit. Exact node outcomes and the terminal pytest collection count are
cross-bound. Network, host-repository fallbacks, user plugins, uncontrolled
environment paths, and dynamic access to downstream receipt/snapshot/evidence
authorities are unavailable. H3.0 registers that future contract; it does not
claim that such a sandbox or subprocess ran while validating this schema.
...
Each development trajectory records the exact latent and observation standard
normal streams plus replayed latent and observation arrays. The validator
regenerates those streams and arrays bitwise, fits the temporally correlated
center posterior from train only, scores every passive validation transition,
and replays every active candidate with common random numbers. Reported
selector scores, costs, and tie outcomes are never authority by themselves.
...
Protected output paths, roles, media types, physical counts, nested schemas,
registered IDs, numeric domains, action-active/null shapes, and cross-file hash
bindings are registered before execution. Every statistic and decision is
recomputed from captured trajectory or SBC bytes. The manifest cannot discover
authority from whatever files happen to exist, and nonempty placeholder JSON
cannot complete. Publication is no-replace, manifest follows complete output
publication, and the result is written last.

The total observed-state classes are armed, consumed-incomplete, completed, and
invalid-terminal. A valid or malformed single-link regular mode-0600 claim
consumes v7 and cannot be retried under another identifier; unsafe claim path
kinds are invalid-terminal and also never armed. Lexical existence includes
broken symlinks and unsafe ancestors. Staging, modes, special files, extras,
omissions, empty completion, and content bindings are validated according to
state.
