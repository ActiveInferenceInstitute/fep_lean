#!/usr/bin/env python3
"""
Stan POMDP/HMM driver: FepLean Symmetric Boolean Generative Model
...
STAN_FILE = Path(__file__).resolve().with_name('FepLean_Symmetric_Boolean_Generative_Model_stan.stan')
RANDOM_SEED = 42
T = 2
S, O, U = 2, 2, 2
PSEUDO_COUNT_STRENGTH = 20.0
NUTS_PARAM_BUDGET = 1024
A = np.asarray([[0.5, 0.5], [0.5, 0.5]], dtype=float)          # O x S, columns sum to 1
B = np.asarray([[[0.5, 0.5], [0.5, 0.5]], [[0.5, 0.5], [0.5, 0.5]]], dtype=float)          # S(next) x S(prev) x U
D = np.asarray([0.5, 0.5], dtype=float)          # S
