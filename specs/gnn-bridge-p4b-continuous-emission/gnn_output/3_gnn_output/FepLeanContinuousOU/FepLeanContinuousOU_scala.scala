...
object FepLeanContinuousOULinearGaussianModelModel {
  ...
  val HToy: H => y = identity
  val QTox: Q => x = identity
  val RToy: R => y = identity
  val prior_covTox: prior_cov => x = identity
  val prior_meanTox: prior_mean => x = identity
