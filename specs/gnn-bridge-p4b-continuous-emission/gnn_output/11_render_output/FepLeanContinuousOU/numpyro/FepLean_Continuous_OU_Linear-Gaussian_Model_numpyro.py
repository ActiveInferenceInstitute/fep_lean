#!/usr/bin/env python3
"""
numpyro continuous (linear-Gaussian state-space) simulation: FepLean Continuous OU Linear-Gaussian Model

Auto-generated by the GNN pipeline — numpyro renderer, continuous branch.
Generative model:
    x_1 ~ N(prior_mean, prior_cov)
    x_t = F x_(t-1) + u_(t-1) + N(0, Q)      (1-dim latent state)
    y_t = H x_t + N(0, R)                    (1-dim observation)
...
NUM_TIMESTEPS = 1
DT = 1.0
OUTPUT_ENV = 'NUMPYRO_OUTPUT_DIR'

F_RAW = [[0.36787944117144233]]
H_RAW = [[1.0]]
Q_RAW = [[0.8646647167633873]]
R_RAW = [[1.0]]
PRIOR_MEAN_RAW = [0.0]
PRIOR_COV_RAW = [[1.0]]
GOAL_MEAN_RAW = None
