#!/usr/bin/env python3
"""
Stan continuous (linear-Gaussian state-space) driver: FepLean Continuous OU Linear-Gaussian Model
...
MODEL_NAME = 'FepLean Continuous OU Linear-Gaussian Model'
STAN_FILE = Path(__file__).resolve().with_name('FepLean_Continuous_OU_Linear-Gaussian_Model_stan.stan')
RANDOM_SEED = 42
T = 1
F = np.asarray([[0.36787944117144233]], dtype=float)
H = np.asarray([[1.0]], dtype=float)
Q = np.asarray([[0.8646647167633873]], dtype=float)
R = np.asarray([[1.0]], dtype=float)
PRIOR_MEAN = np.asarray([0.0], dtype=float)
PRIOR_COV = np.asarray([[1.0]], dtype=float)
GOAL_MEAN = None
CONTROL_GAIN = None
