...
def _get_latest_verification_manifest(project_root: Path) -> Path | None:
    candidates = list((Path(project_root) / "output" / "reports").glob("*/verification_manifest.json"))
...
def _hermes_block_from_summary(path: Path | None) -> dict[str, Any]:
    ...
    keys.update({
        ...
        "primary_model": max(counts, key=counts.get) if counts else "",
        "models_used": ", ".join(sorted(set(models))),
        "model_fallback_count": max(0, len(models) - counts.get(max(counts, key=counts.get), 0)) if counts else 0,
...
def build_manuscript_vars(catalogue: FEPTopicCatalogue, project_root: Path) -> dict[str, Any]:
...
def write_manuscript_vars(project_root: Path, catalogue: FEPTopicCatalogue | None = None) -> Path:
    root = Path(project_root)
    cat = catalogue or FEPTopicCatalogue.from_yaml(root / "config" / "topics.yaml")
    out = root / "manuscript" / "manuscript_vars.yaml"
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(yaml.safe_dump(build_manuscript_vars(cat, root), sort_keys=False, allow_unicode=True), encoding="utf-8")
    return out
