...
def _setup(root: Path) -> int:
    ...
    if not lake:
        ...
        if not bootstrap.is_file():
            print("lake executable is unavailable and the bootstrap script is missing", flush=True)
            return 1
        bootstrap_env = dict(os.environ)
        elan_home = bootstrap_env.get("ELAN_HOME", str(Path.home() / ".elan"))
        bootstrap_env["ELAN_HOME"] = elan_home
        bootstrap_env["PATH"] = str(Path(elan_home) / "bin") + ":" + bootstrap_env.get("PATH", "")
        try:
        ...
        if result.returncode:
            print(f"setup failed with exit code {result.returncode}: {bootstrap}", flush=True)
    ...
    for command in ((lake, "update"), (lake, "exe", "cache", "get"), (lake, "build")):
        try:
            ...
            result = subprocess.run(command, cwd=lean_dir, env=env, timeout=remaining, check=False)
        except (OSError, subprocess.TimeoutExpired) as exc:
            print(f"setup failed: {exc}", flush=True)
            return 1
        if result.returncode:
...
def _verify(root: Path, topic_ids: list[str] | None, area: str | None) -> int:
    ...
    try:
        ...
        print(json.dumps({"status": "error", "mode": "lean-only", "complete": False, "failure_reason": str(exc)}))
        return 1

    verifier = LeanVerifier(lean_dir=root / "lean", project_root=root)
    mathlib_ok, mathlib_message = verifier.check_mathlib_built()
    if not mathlib_ok:
        print(json.dumps({"status": "error", "mode": "lean-only", "complete": False, "catalogue_topics": len(topics), "verified_topics": 0, "mathlib": mathlib_message, "results": []}, indent=2))
        return 1
    ...
    print(json.dumps({"status": "ok" if complete else "error", "mode": "lean-only", "complete": complete, "catalogue_topics": len(topics), "verified_topics": clean, "mathlib": mathlib_message, "results": result_payload}, indent=2, default=str))
