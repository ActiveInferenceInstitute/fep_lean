...
Manuscript injection for Pandoc: builds YAML consumed as `manuscript/manuscript_vars.yaml` and regenerates the unified formalism appendix `09z_unified_formalism_catalogue.md` (Lean + typeset LaTeX per topic: `{#sec:…}` section anchors and displayed `equation` + `\label{eq:…}` for cross-refs).
...
### `build_unified_formalism_appendix_markdown` / `write_unified_formalism_appendix_markdown`

```python
def build_unified_formalism_appendix_markdown(
    catalogue: FEPTopicCatalogue, project_root: Path
) -> str
