...
**Version**: v0.7.1 | **Native verify**: 50/50 `lake env lean` (green sweep) | **Mathlib4**: v4.29.0 | **Model**: moonshotai/kimi-k2.6 (Hermes primary; `z-ai/glm-5.1` demoted to fallback)
...
- **Tests**: [`../tests/`](../tests/) — 29 `test_*.py` modules, **347** collected tests (latest `uv run pytest --collect-only -q` from `projects/fep_lean/`), coverage exceeds the **89%** gate (`pyproject.toml::fail_under = 89`; see [`tests/AGENTS.md`](../tests/AGENTS.md))
- **Lean workspace**: [`../lean/`](../lean/) (Lake + Mathlib v4.29.0, pinned in `lakefile.lean`)
- **Manuscript**: [`../manuscript/`](../manuscript/) — 24 numbered Markdown chapters + `AGENTS.md` + `README.md` + `preamble.md` + `config.yaml` + `manuscript_vars.yaml` + `references.bib` (30 files total under `manuscript/`); **Manuscript Artifacts** writes `manuscript_vars.yaml` via `write_manuscript_vars` and `09z_unified_formalism_catalogue.md` via `write_unified_formalism_appendix_markdown` (Lean + typeset LaTeX: `{#sec:…}` anchors and `equation` + `\label{eq:…}` for cross-refs; see [`xref_audit.py`](xref_audit.py)). Optional one-shot placeholder injection: [`../scripts/_inject_manuscript_vars.py`](../scripts/_inject_manuscript_vars.py).
...
> **Benchmark note**: Full end-to-end runs with `FEP_LEAN_GAUSS_WORKFLOWS=1` are LLM-latency dominated (order tens of minutes depending on keys and model — current primary `moonshotai/kimi-k2.6` is a reasoning model). Native compilation headline is **`50/50`** after a green verifier sweep; see `manuscript_vars.yaml` / `verification_manifest.json`.

This folder (`docs/`, beside `src/` and `manuscript/`) holds the hand-maintained reference docs *about* the project. For the narrative paper content go to `../manuscript/`; for the tech contracts that agents must honour read `../AGENTS.md`; for the system functional spec read `../SPEC.md`.
...
| [cold-start-and-cleanup.md](cold-start-and-cleanup.md) | What to delete for a fresh `output/` + manuscript artifacts; optional `GAUSS_HOME` / `lean/.lake/` |
...
| [pin_audit.py](pin_audit.py) | Flag Lean / Mathlib / Hermes primary-model literals in `*.md` that drift from `lean/lean-toolchain`, `lean/lakefile.lean`, `config/settings.yaml` |
| [xref_audit.py](xref_audit.py) | Ensure every `\ref` / `\eqref` in `manuscript/*.md` resolves to `{#id}` or `\label{id}` |

### Generated at runtime

| Path | Source |
| ---- | ------ |
| [`../manuscript/09z_unified_formalism_catalogue.md`](../manuscript/09z_unified_formalism_catalogue.md) | Auto-written by `output.manuscript.write_unified_formalism_appendix_markdown` (B+C: Lean + LaTeX with `{#sec:…}` anchors and `equation` + `\label{eq:…}`) — **do not edit** |
...
# Pin drift (static docs vs canonical toolchain / primary model) and manuscript xref integrity
uv run python pin_audit.py
uv run python xref_audit.py
```

All six scripts exit **0** when clean and **1** on any issue — suitable for CI gates. After editing [pipeline.md](pipeline.md) or [configuration.md](configuration.md), run `check_links.py --strict` so new `#anchor` targets are validated (see [docs/AGENTS.md](AGENTS.md)).
