...
A per-topic tag (`mathlib_status`) with values `real` / `partial` / `aspirational`. In v0.7.1 **all 50 topics are `real`** — they compile cleanly with no `sorry` on Lean v4.29.0 + Mathlib v4.29.0.
