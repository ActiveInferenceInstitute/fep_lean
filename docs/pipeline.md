...
1. **`FEPPipeline.run()`** — Load Catalogue → Environment Validation → Gauss Sessions (**skipped** if workflows falsy) → Manuscript Artifacts (`write_manuscript_vars`, unified `09z_unified_formalism_catalogue.md` with `{#sec:…}` anchors and `equation` + `\label{eq:…}`, `write_all_catalogue_figures` → `output/figures/`). Optional env **`FEP_LEAN_MAX_TOPICS`** caps how many topics enter Gauss Sessions after filters.
...
| 4 | Manuscript Artifacts | `write_manuscript_vars` + `write_unified_formalism_appendix_markdown` (`09z_unified_formalism_catalogue.md`: `{#sec:…}` + `equation` / `\label{eq:…}`) + `write_all_catalogue_figures` |
...
- **Thread 1** runs `_manuscript_block`, which writes `manuscript/manuscript_vars.yaml` via
  `write_manuscript_vars` and `manuscript/09z_unified_formalism_catalogue.md` via
  `write_unified_formalism_appendix_markdown`. The manuscript block is pinned to a single thread so
  it does not race figure generation.
...
- [AGENTS.md](AGENTS.md) — documentation audit gates (`check_links`, `md_hygiene`, `pin_audit`, `xref_audit`)
