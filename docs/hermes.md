...
**Canonical catalogue text for the paper** remains **`config/topics.yaml`** and the regenerated **`manuscript/09z_unified_formalism_catalogue.md`** (Lean + typeset LaTeX per topic; pandoc `{#sec:…}` section anchors and `\label{eq:fep-NNN-k}` inside displayed equations for cross-refs). Hermes output is **session / audit** material unless you merge changes back into YAML.
...
> - **Cross-model chain advances**: the live count is reported as `hermes.model_fallback_count / hermes.processed` in `manuscript/manuscript_vars.yaml`, with a per-reason breakdown in `hermes.chain_advance_reasons` (categories: `empty_content`, `wall_clock_timeout`, `transport_error`, `non_retriable_http`, `parse_error`).  See `manuscript/03d_hermes_llm_pipeline.md § Three Classes of Fallback` for the taxonomy.
> - **Same-model network retries**: `hermes.network_retry_count` (sum of bounded `429` / transport retries inside `_try_fetch_raw` that did *not* advance the OpenRouter chain). `HERMES_429_MAX_RETRIES=2` and `HERMES_NETWORK_MAX_RETRIES=2` are sufficient in practice.
...
| 7 | `nousresearch/hermes-3-llama-3.1-405b:free` | 405B NL-strong; 8 req/min on free tier |
| 8 | `arcee-ai/trinity-large-preview:free` | Preview reasoning-focused |
