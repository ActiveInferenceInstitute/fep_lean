...
## Stale `output/` or missing `manuscript_vars.yaml` after a manual cleanup

**Symptoms**: Broken internal links in `docs/README.md` (strict `check_links`), or manuscript `{{…}}` not substituting.

**Cause**: `manuscript/manuscript_vars.yaml` and `manuscript/09z_unified_formalism_catalogue.md` are generated files; deleting them is correct for a cold start but they must be recreated.

**Fix**: See [cold-start-and-cleanup.md](cold-start-and-cleanup.md) — full pipeline rerun or the short `PYTHONPATH=src:. uv run python -c "..."` block for catalogue-only regeneration.
...
Expected good state: preflight exits 0, `uv sync` installs without errors, test suite reports **347** collected (346 pass, 1 API-key-gated skip) (`uv run pytest --collect-only -q` from `projects/fep_lean/`) with **≥89%** combined line+branch coverage on `src/` per `pyproject.toml::fail_under` (run pytest from this directory so `[tool.coverage.run] source = ["src"]` applies).
...
## 13. Generated unified appendix (`09z_unified_formalism_catalogue.md`) not updating

**When you see it**: You edited `config/topics.yaml` (or regenerated `latex_equations`) and the formalism appendix still shows old content.

**Root cause**: The unified appendix is **auto-generated** on every pipeline run from `src/output/manuscript.py` (`write_unified_formalism_appendix_markdown`). It is in `.gitignore` so you do not commit it directly.
...
# Appendices are rewritten; re-render the PDF (from repository root):
...
## `pin_audit.py` or `xref_audit.py` exits 1 after a docs edit

**Symptoms**: From `docs/`, `uv run python pin_audit.py` prints `drift` lines, or `uv run python xref_audit.py` lists `unresolved \ref{...}`.

**`pin_audit` cause**: A static markdown file (outside `manuscript/`, which uses `{{…}}` placeholders) mentions a Lean toolchain string, Mathlib `vX.Y.Z`, or `moonshotai/kimi-k2.*` that no longer matches [`../lean/lean-toolchain`](../lean/lean-toolchain), [`../lean/lakefile.lean`](../lean/lakefile.lean), or [`../config/settings.yaml`](../config/settings.yaml) (`hermes.model`).

**`pin_audit` fix**: Update the prose to the canonical literal the script prints, *or* bump the SSOT files first if you are intentionally upgrading the pin, then sweep the docs.

**`xref_audit` cause**: A manuscript chapter uses `\ref{eq:foo}` / `\eqref{eq:foo}` without a matching `{#eq:foo}` pandoc anchor or `\label{eq:foo}` in a displayed equation.

The auto-generated `manuscript/09z_unified_formalism_catalogue.md` usually defines equation targets with `\label{eq:…}` inside `equation` blocks and uses `{#sec:…}` for per-topic section splits; `{#eq:foo}` remains a valid alternate style in hand-authored chapters.

**`xref_audit` fix**: Add the missing label/anchor on the equation the prose intends, or rewrite the sentence to cite the surviving label. See [docs/AGENTS.md](AGENTS.md) and [docs/SPEC.md](SPEC.md) (invariants **B5** / **B6**).
