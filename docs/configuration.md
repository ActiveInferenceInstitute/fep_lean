...
> **Note:** `manuscript/config.yaml::paper.version` is the **manuscript** semver (currently **0.7.1** after Tier 8–9). `project.version` here tracks the **Python package** and should stay aligned with [`pyproject.toml`](../pyproject.toml); they may differ by patch until a coordinated release bumps both.
...
| `version` | str | `0.7.1` | Semantic version (Python package; must match `pyproject.toml`; bump in lockstep) |
...
| `version` | str | `0.7.1` | Manuscript `paper.version` (align with docs release; often matches `pyproject.toml` / `config/settings.yaml::project.version` after a coordinated bump) |
...
| 7 | `nousresearch/hermes-3-llama-3.1-405b:free` | 131K | 405B NL-strong; 8 req/min limit on free tier |
| 8 | `arcee-ai/trinity-large-preview:free` | 131K | Preview reasoning-focused |
...
- **version**: `0.7.1` (bump with `config/settings.yaml::project.version` in lockstep; manuscript `paper.version` may lead by patch)
